...
void CoherentPointDrift::setInitP(const vnl_matrix<double>& PC) {
	constrained = true;
	this->PC.set_size(PC.rows(), PC.cols());//fixme bug
	this->PC = PC;
}

int CoherentPointDrift::PrepareInput(const vnl_matrix<double>& scene,
		const vnl_matrix<double>& model) {
	Base::PrepareInput(scene, model);
	ctrl_pts_ = model_;
	n_ = ctrl_pts_.rows();
...
int CoherentPointDrift::PrepareInput(const char* f_config) {
	...
	GetPrivateProfileString(common_section_, "ctrl_pts", NULL, f_ctrl_pts, 80,
			f_config);
...
void CoherentPointDriftTps::PrepareBasisKernel() {
	...
	vnl_matrix<double> A = G.transpose() * Q2 * Q2.transpose() * G
			+ lambda_ * kernel_;
	vnl_qr<double> qr2(A);
...
const vnl_matrix<double>& CoherentPointDrift::getTransformedModel() {
	return transformed_model_;
}

const vnl_matrix<double>& CoherentPointDrift::getP() {
	return P;
}
...
void CoherentPointDrift::StartRegistration(vnl_vector<double>& params) {
	...
	while ((iter < max_iter) && (ntol > tol)) {
		...
		while ((EMiter < max_em_iter) && (EMtol > tol)) {
			...
			E_old = E;
			ComputeP(moving_model, scene_, P, Eu, sigma_, outliers, constrained,
					PC);
			bending = UpdateParam();
...
int CoherentPointDrift::SetInitParams(const char* f_config) {
	...
	GetPrivateProfileString("Files", "init_params", NULL, f_init_params, 80,
			f_config);
...
void CoherentPointDrift::SaveResults(const vnl_vector<double>& params) {
	SaveTransformed(params);
}
...
void CoherentPointDrift::PrepareOwnOptions(const char* f_config) {
	...
	GetPrivateProfileString(section_, "outliers", "0", s_outliers, 60,
			f_config);
