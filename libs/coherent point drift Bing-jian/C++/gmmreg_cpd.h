...
public:
	...
	const vnl_matrix<double>& getTransformedModel();
	const vnl_matrix<double>& getP();
	void setInitP(const vnl_matrix<double>& P);
...
private:
	...
	void SaveResults(const vnl_vector<double>&) override;
	int PrepareInput(const char*) override;
	int PrepareInput(const vnl_matrix<double>& scene, const vnl_matrix<double>& model) override;
