...
void Base::Run(const char* f_config, const vnl_matrix<double>& scene,
		const vnl_matrix<double>& model) {
	Initialize(f_config, scene, model);
...
int Base::Initialize(const char* f_config, const vnl_matrix<double>& scene,
		const vnl_matrix<double>& model) {
	PrepareInput(scene, model);
...
int Base::PrepareInput(const vnl_matrix<double>& scene,
		const vnl_matrix<double>& model) {
	model_ = model;
	scene_ = scene;
	m_ = model_.rows();
...
int Base::PrepareInput(const char* f_config) {
	...
	GetPrivateProfileString(common_section_, "model", NULL, f_model, 256,
			f_config);
	if (LoadMatrixFromTxt(f_model, model_) < 0) {
		return -1;
	}
	GetPrivateProfileString(common_section_, "scene", NULL, f_scene, 256,
			f_config);
...
void Base::SaveTransformed(const vnl_vector<double>& params) {
...
void Base::SaveTransformed(const char* filename,
	...
	int num = GetPrivateProfileInt(section_correspondence, "num_of_thresholds",
			0, f_config);
	if (num > 0) {
		char s_min[256], s_max[256], s_pairs[256];
		GetPrivateProfileString(section_correspondence, "min_threshold", NULL,
				s_min, 255, f_config);
		GetPrivateProfileString(section_correspondence, "max_threshold", NULL,
				s_max, 255, f_config);
		GetPrivateProfileString(section_correspondence, "matched_pairs", NULL,
				s_pairs, 255, f_config);
		...
		for (int i = 0; i < num; ++i) {
			...
			f_pair << "# of matched point pairs : " << pairs.cols()
					<< std::endl;
			int j;
	...
	/*std::cout << "Please find the transformed model set in " << filename
			<< std::endl;*/
}
...
void Base::MultiScaleOptions(const char* f_config) {
	...
	GetPrivateProfileString(section_, "max_function_evals", NULL, s_func_evals,
			255, f_config);
