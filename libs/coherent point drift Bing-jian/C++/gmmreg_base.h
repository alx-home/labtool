...
public:
	...
	void Run(const char* f_config, const vnl_matrix<double>& scene, const vnl_matrix<double>& model);
...
protected:
	...
	virtual int PrepareInput(const vnl_matrix<double>& scene, const vnl_matrix<double>& model);
	int SetCtrlPts(const char* filename);
	void SaveTransformed(const char* filename, const vnl_vector<double>&, const char* f_config);
	void SaveTransformed(const vnl_vector<double>&);
	void MultiScaleOptions(const char* f_config);
	void DenormalizeAll();
	int Initialize(const char* f_config);
	int Initialize(const char* f_config, const vnl_matrix<double>& scene, const vnl_matrix<double>& model);
...
private:
	...
	virtual void SaveResults(const char* filename, const vnl_vector<double>&) = 0;
	virtual void SaveResults(const vnl_vector<double>&) {
	}
	;
