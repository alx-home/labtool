...
//  { "EMG",                    "uV",    "%.1fuV",          "%.1f", (float)0.10  },
  ...
  void FileFormatMDF::Write(const MotionFileHandler& /*context*/, const TreeValue* contents, std::ostream& os, const TreeValue* writeoptions) const throw(MotionFileException)
    ...
    if (subject_data_count >= 5)
    {
      ...
      s = (strcasecmp(subject.Gender, "male") == 0) ? 1 : 0;
      os.write((const char*)&wElements,2);
    ...
    if (gait_event_group)
    {
      ...
      for (size_t event_index = 0; event_index < num_events; event_index++)
      {
        ...
        // left cycle
        if ((flagL.size() == 0 && strcasecmp(name.c_str(),GaitEvents::LStart) == 0) ||
            (flagL.size() == 1 && strcasecmp(name.c_str(),GaitEvents::LToeOff) == 0) ||
            (flagL.size() == 2 && strcasecmp(name.c_str(),GaitEvents::LEnd) == 0))
        {
          flagL.push_back(t);
        }

        // right cycle
        if ((flagR.size() == 0 && strcasecmp(name.c_str(),GaitEvents::RStart) == 0) ||
            (flagR.size() == 1 && strcasecmp(name.c_str(),GaitEvents::RToeOff) == 0) ||
            (flagR.size() == 2 && strcasecmp(name.c_str(),GaitEvents::REnd) == 0))
        {
          flagR.push_back(t);
        }
      }
